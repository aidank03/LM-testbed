...
def main(argv=None):
    ...
    suite.add_argument("--seed", type=int, default=20260906)
    suite.add_argument("--contract-version", type=int, choices=(1, 2), default=2)
    ...
    try:
        if args.command == "doctor":
            ...
            if args.contract_version == 2:
                from .evaluation_v2 import create_suite
            else:
            ...
            schema = read(directory / "public_cases.json").get("schema_version")
            if schema == "factor-cases/2":
                from .evaluation_v2 import baseline_results, compare_methods, _validate_public
            else:
