...
EXAMPLES = {
    "recorded": "TARGET: detector voltage. The acquisition system saved detector voltage samples directly.",
    "reconstructed": "TARGET: radial velocity. Signal processing converted the saved PDV waveform into radial velocity.",
    "model_inferred": "TARGET: material temperature. A simulation fit to the measured signal estimated material temperature.",
    "unresolved": "TARGET: ETI caused the fingers. Images show fingers, but no observation distinguishes ETI from competing mechanisms.",
}
...
class ProvenanceModule(nn.Module):
    ...
    @torch.inference_mode()
    def score_labels(self, text):
        """Compare exact-label sequence probabilities; these are not calibrated confidence."""
        self.eval()
        prefix = self.prompt_tokens(text)
        scores = []
        for label in self.labels:
            suffix = self.tokenizer.encode(label, add_special_tokens=False) + [self.tokenizer.eos_token_id]
            tokens = prefix + suffix
            ids = torch.tensor([tokens], device=self.device_name)
            labels = torch.tensor([[-100] * len(prefix) + suffix], device=self.device_name)
            output = self.lm(input_ids=ids, attention_mask=torch.ones_like(ids),
                             labels=labels, use_cache=False)
            # Hugging Face causal-LM loss averages the unmasked next-token losses.
            log_probability = -float(output.loss) * len(suffix)
            scores.append(log_probability)
        preferences = torch.softmax(torch.tensor(scores, dtype=torch.float64), dim=0).tolist()
        ranking = sorted(zip(self.labels, preferences), key=lambda item: item[1], reverse=True)
        return {"ranking": [{"label": label, "relative_preference": round(value, 4)}
                            for label, value in ranking],
                "warning": "Relative preference among four exact answers; not calibrated confidence or physical probability"}

    def token_view(self, text):
        tokens = self.prompt_tokens(text)
        return {"prompt_tokens": len(tokens), "first_token_ids": tokens[:24],
                "first_pieces": [self.tokenizer.decode([token]) for token in tokens[:24]],
                "note": "Tokens are model text pieces, not words or physical measurements"}
...
def main():
    ...
    parser.add_argument("command", nargs="?", choices=["interactive", "info", "predict", "score", "tokens", "train"], default="interactive")
    ...
    if args.command == "info":
        ...
        if args.command in {"predict", "score", "tokens"}:
            action = {"predict": model.predict, "score": model.score_labels,
                      "tokens": model.token_view}[args.command]
            print(json.dumps(action(args.text), indent=2))
            return
        print("Enter a record and TARGET claim. Type /help to see experiments.", flush=True)
        last_text = None
        while True:
            try:
                ...
                if text == "/quit":
                    break
                if text == "/help":
                    print("/examples  four known examples\n/score     compare label preferences for your last input")
                    print("/tokens    show how your last input became token numbers\n/info      device and weights\n/quit      close")
                    continue
                if text == "/examples":
                ...
                if text == "/info":
                    ...
                    continue
                if text == "/example":
                    text = EXAMPLES["unresolved"]
                    print(text)
                if text == "/score":
                    print(json.dumps(model.score_labels(last_text), indent=2) if last_text else "Enter a scientific note first.")
                    continue
                if text == "/tokens":
                    print(json.dumps(model.token_view(last_text), indent=2) if last_text else "Enter a scientific note first.")
                    continue
                if text:
                    last_text = text
